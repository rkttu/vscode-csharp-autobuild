...
Before uploading a platform, the publisher creates a draft GitHub release containing the tested bytes. It submits each missing target once, then allows up to 15 minutes for Open VSX's asynchronous publication to make the accepted uploads publicly available. Every downloaded hash must still match; a pending or rejected version cannot pass the gate. If an upload stops after some targets succeed, a later run restores the draft assets, validates them again, skips matching registry bytes, and uploads only missing targets. Automatic discovery finishes preserved drafts before building new source/recipe candidates. If all uploads succeeded but GitHub finalization failed, the retry completes finalization without uploading again. A changed file behind an existing version or a missing preserved asset fails the retry. Publisher tests simulate these cases without altering the public registry.
