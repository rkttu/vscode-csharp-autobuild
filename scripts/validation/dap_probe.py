"""Native-architecture DAP launch probe derived from the archived macOS experiment."""
import argparse
import json
import os
...
class Dap:
    ...
    def wait(self, predicate, timeout=20):
        ...
        while True:
            ...
            try:
                ...
                raise TimeoutError('DAP message timeout') from error
    ...
    def close(self):
        if self.proc.poll() is None:
            try:
                seq = self.send('disconnect', {'terminateDebuggee': True})
                self.wait(lambda m: m.get('type') == 'response' and m.get('request_seq') == seq, timeout=3)
...
def run(args):
    ...
    result = {'engine': args.engine, 'program': args.program,
              'expectedArchitecture': args.expected_arch, 'expectedRuntime': args.expected_runtime,
              'checks': []}
    try:
        ...
        assert exception_filter, f'No supported exception breakpoint filter: {filter_ids}'
        ...
        d.event('terminated')
        expected_arch = {'x64': 'X64', 'arm64': 'Arm64'}[args.expected_arch]
        assert all(value in d.output for value in [f'ARCH={expected_arch}',
                   f'RUNTIME=.NET {args.expected_runtime}.', 'ANSWER=42', 'AFTER_AWAIT=43', 'CAUGHT', 'DONE']), d.output
