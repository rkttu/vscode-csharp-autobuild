...
The current C# manifest uses `images/csharpIcon.png`, the purple C# hexagon. No alternate extension icon was found in that upstream source snapshot. The initial variant used a charcoal rounded square with C# lettering, an amber breakpoint and a stepping arrow. The [historical asset](https://github.com/rkttu/vscode-csharp-autobuild/blob/0c0cc76ecda64d6092fb79179d65baa841f60588/assets/netcoredbg/icon.png) belonged to candidate `0.1.4000`; the current asset notes describe its later replacement. [Upstream manifest](https://github.com/dotnet/vscode-csharp/blob/2f5806a9f39575bfaf4ca16445f420440a43e050/package.json), [asset notes](../../assets/netcoredbg/README.md)
...
After those fixes, Alpine x64 and ARM64 still crashed with process exit `-11` (`SIGSEGV`) on both runtime families. Core dumps show `ManagedCallback::CreateProcessW` calling `Interop::Init`, followed by `coreclr_initialize` inside the installed runtime. This is consistent with the independently reported musl hosting/stack-initialization failure in dotnet/runtime #103741. The downloaded runtime libraries were stripped, so the exact internal crashing CoreCLR function was not symbolized in this run. [Archived crash diagnosis](cross-platform-2026-09-06/alpine-crash/), [runtime issue and maintainer discussion](https://github.com/dotnet/runtime/issues/103741)
...
Public release jobs run only on `main` after the preceding gates pass. Research-branch manual runs use a dry run; the temporary research push trigger was removed after the successful candidate run. The publisher preserves all VSIX files, debugger ZIPs, manifests and native evidence in a separate draft GitHub release before Open VSX uploads. This initial candidate used `csharp-netcoredbg-v...` tags and an independent `0.1` version prefix with a scheduler-run-based numeric patch. The later [accepted version policy](2026-09-06-versioning-and-merge-readiness.md) replaces both formats. Releases retain `--latest=false`, protecting the existing Latest marker. [Candidate workflow](../../.github/workflows/netcoredbg-candidate.yml), [publisher](../../scripts/variant/publish.py)
