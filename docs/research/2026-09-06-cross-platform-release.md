...
On September 6, 2026, this repository built Samsung netcoredbg 3.2.0-1092 for all eight C# extension targets. All 498 original Samsung files remained unchanged on every target. Windows, glibc Linux and macOS passed the .NET 8 and .NET 10 DAP checks on both CPUs. Alpine compiled but crashed during runtime initialization on both CPUs. The aggregate gate therefore blocked VSIX generation and publication. [Complete native run](https://github.com/rkttu/vscode-csharp-autobuild/actions/runs/34019887224), [candidate pipeline](https://github.com/rkttu/vscode-csharp-autobuild/actions/runs/34019887593)
...
> Checked on September 6, 2026. The research branch has not been merged into the default branch. No new extension has been published. After the initial complete matrix failed, a separate Alpine hosting experiment passed all four runtime combinations. A fresh complete matrix and real VSIX packaging remain the next gates.
...
The complete run used Samsung commit `9744e1f051866215611b8440c638042aa2aa2f72`, CoreCLR input commit `60629d14374c56f1cb51819049ad1fa529307f8d`, SDKs 8.0.418 and 10.0.400, and DbgShim 10.0.731102. The test processes reported .NET 8.0.24 and .NET 10.0.11. These are pinned test inputs and observed runtimes, not assertions about the latest servicing releases. [Build configuration](../../config/netcoredbg.json), [archived results](cross-platform-2026-09-06/summary.json)
...
| `alpine-x64` | Alpine 3.22.1 container on native x64 | Passed with external build compatibility inputs | Failed at runtime initialization | Failed at runtime initialization |
| `alpine-arm64` | Alpine 3.22.1 container on native ARM64 | Passed with external build compatibility inputs | Failed at runtime initialization | Failed at runtime initialization |

Each passing runtime combination completed eight checks: conditional breakpoint, stack, variables, evaluation, stepping, post-await breakpoint, exception information and expected output with clean termination. Thus twelve of sixteen required runtime combinations passed. The unchanged-source audit covered all 498 tracked Samsung input files on all eight targets. Generated files and the separate CoreCLR input tree are outside that file-preservation audit. [Validation scripts](../../scripts/validation/README.md)
...
Local checks passed nine negative/positive aggregate tests, six detector/recovery-selection tests, an actual source-overlay smoke test, the isolated factory tests and upstream `npm run compile` (TypeScript, ESLint, localization and Razor grammar generation). The overlay smoke used explicitly synthetic validation metadata and did not create a release-eligible package. Workflow syntax passed actionlint. The real candidate run independently demonstrated that Alpine failure skips packaging, VSIX tests and publication.

The next release decision depends on Alpine runtime compatibility and the first successful package/adapter integration run. Until then, the branch provides working source-build verification and a guarded release implementation, not an operational eight-platform published extension. No registry write, default-branch merge or change to the existing extension's production workflow has occurred.
